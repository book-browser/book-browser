...
public class PartyService {
	...
	public PageDto<PartyDto> findAll(PartySearchCriteriaDto partySearchCriteriaDto) {
		...
		Specification<Party> specification = PartySpecification.orderBy(partySearchCriteriaDto.getSort(),
				partySearchCriteriaDto.getOrder());

		if (partySearchCriteriaDto.getName() != null) {
			specification = specification.and(PartySpecification.fullNameLike(partySearchCriteriaDto.getName().get()));
		}
		return new PageDto<>(partyRepository.findAll(specification, pageable)
				.map(PartyService::convertPartyToPartyDto));
	}
	...
	public PageDto<PartyDto> findAllPublishers(PartySearchCriteriaDto partySearchCriteriaDto) {
		...
		if (partySearchCriteriaDto.getName() != null) {
			specification = specification.and(PartySpecification.fullNameLike(partySearchCriteriaDto.getName().orElse("")));
		}
