	...
	public static Specification<Series> orderBy(String field, OrderEnum order) {
		return (series, cq, cb) -> {
			...
			if (field.equals("title")) {
				expression = cb.function("replace", String.class, cb.upper(series.get("title")), cb.literal("THE "),
						cb.literal(""));
			}
	...
	public static Specification<Series> titleStartsWith(LetterEnum letterEnum) {
		return (series, cq, cb) -> {
			...
			return cb.equal(cb.function("regexp_like", String.class,
					cb.function("replace", String.class, cb.upper(series.get("title")), cb.literal("THE "), cb.literal("")),
