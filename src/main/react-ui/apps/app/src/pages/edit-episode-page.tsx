      ...
      {episode && (
        <div>
          <Breadcrumb>
            ...
            <Breadcrumb.Item linkAs={Link} linkProps={{to: "/episodes"}}>Episodes</Breadcrumb.Item>
            <Breadcrumb.Item linkAs={Link} linkProps={{to: `/series/${episode.seriesId}`}}>{episode.seriesTitle}</Breadcrumb.Item>
            <Breadcrumb.Item linkAs={Link} linkProps={{to: `/episode/${episode.id}`}}>{(savedEpisode || loadedEpisode)?.title}</Breadcrumb.Item>
          ...
          <EpisodeForm
            ...
            footer={
              <div>
                ...
                {!saveError && savedEpisode && <Alert variant="success" className="mb-2">Changes successfully saved</Alert>}
                <div className="d-flex">
                  <Button className="mr-2" variant="secondary" disabled={savingEpisode} onClick={cancel}>Cancel</Button>
                  <Button className="mr-auto" variant="secondary" disabled={savingEpisode} onClick={reset}>Reset</Button>
                  <Button variant="primary" type="submit" disabled={savingEpisode}>
                    {!savingEpisode && <span>Save</span>}
                    {savingEpisode && <span>Saving <CircularProgress color="secondary" size={"15px"} /></span>}
                  </Button>
                </div>
              </div>
            }
