        ...
        <Card.Body>
          ...
          <RegisterForm
            ...
            footer={
              <div>
                {error && (
                  <ErrorAlert uiMessage="Something went wrong. Unable to register." className="mb-3" error={error} />
                )}
                {!loading && (
                  <Button className="mb-4" variant="primary" type="submit" disabled={!valid}>
                    Register
                  </Button>
                )}
                {loading && (
...
const RegisterPage = () => {
  ...
  return (
    <>
      {(!executed || error) && (
        <RegisterStep valid={valid} loading={loading} error={error} onChange={onChange} onSubmit={onSubmit} />
      )}
